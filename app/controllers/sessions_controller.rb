class SessionsController < ApplicationController
  ...
  def index  
    ...
    when "title"
    @notes = @current_user.notes.where("title ilike ?", "%#{params["search-terms"]}%").order('created_at DESC')
    when "content"
    @notes = @current_user.notes.where("content ilike ?", "%#{params["search-terms"]}%").order('created_at DESC') 
    else   
